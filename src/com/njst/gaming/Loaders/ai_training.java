...
public class ai_training implements SceneLoader {
    ...
    public void load(Scene scene) {
        // Create enhanced AI training environment with proper skybox, groundplane, and
        // body model
        ...
        // int bodyTexture = ShaderProgram.loadTexture(data.rootDirectory+"/P.png");
    ...
    private void loadBodyModelWithBones(Scene scene) {
        try {
            ...
            Map<String, KeyframeAnimation> fbxanims = FBXAnimationLoader.extractAnimation(
                    data.rootDirectory + "/Defeated.fbx", 0,
                    100);
            // Map<String, KeyframeAnimation> fbxanims1 =
            // FBXAnimationLoader.extractAnimation(data.rootDirectory+"/Defeated.fbx", 1,
            // 100);

            // Load bone hierarchy
            Bone rootBone = FBXBoneLoader.loadBones(data.rootDirectory + "/Defeated.fbx", fbxanims, 1.0f);
            ...
            int[] fbx_textures = FBXBoneLoader.load_Textures(data.rootDirectory + "/P.fbx");
            Weighted_GameObject bodyModel = new Weighted_GameObject(
                    FBXBoneLoader.loadModel(data.rootDirectory + "/Defeated.fbx", bones, 0, 1.0f),
                    fbx_textures[0]);
            ...
            for (int i = 0; i < 6; i++) {
                ...
                Map<String, KeyframeAnimation> temp_anims = FBXAnimationLoader.extractAnimation(
                        data.rootDirectory + "/Defeated.fbx",
                        animationIndex, 100);
