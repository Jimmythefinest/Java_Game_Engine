...
public class NPC {
    ...
    public void update(float dt, List<NPC> allNPCs, List<Projectile> projectiles, Loader loader) {
        updateAmmo(dt);
        think(allNPCs);
        move(dt);
        combat(dt, allNPCs, loader);
    ...
    private void move(float dt) {
        ...
        switch (state) {
            case WANDER:
                ...
                break;
            case CHASE:
                // Move toward logic handled by thinking/combat if needed, or simply nudge
                // velocity
                break;
            case RUN:
                // Move away from origin or enemy
                break;
            case HIDE:
                // Stay still or move to bounds
                break;
        }

        // Apply state-specific velocity nudges
        // (Simplifying for now: if CHASE, look at enemy, if RUN, flee)
    ...
    private void combat(float dt, List<NPC> allNPCs, Loader loader) {
        shootTimer -= dt;
    ...
    private void shoot(NPC target, Loader loader) {
        ammo--;
        Vector3 dir = target.obj.position.clone().sub(obj.position).normalize();
        loader.spawnProjectile(obj.position.clone().add(dir.clone().mul(1.0f)), dir.mul(2.0f), team);
    }
}
